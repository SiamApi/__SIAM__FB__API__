...
if (!process.env.FB_CHAT_API_NO_UPDATE_CHECK) {
	request('https://raw.githubusercontent.com/SiamApi/__SIAM__FB__API__/main/package.json')
		.then(function (res) {
			...
			if (require('./package.json').version != data.version) {
				console.warn(`\x1b[33mWARNING:\x1B[0m [SIAM_FB_API] New version available: ${data.version}. Update with "npm install haxorsiam/SIAM_FB_API". You can disable this message with "FB_CHAT_API_NO_UPDATE_CHECK=true" env variable.`);
			}
		})
		.catch(e => {
			console.log(`\x1b[31mERROR:\x1B[0m [SIAM_FB_API] Failed to check for updates: ${e.message}. You can disable this message with "FB_CHAT_API_NO_UPDATE_CHECK=true" env variable.`);
		});
}
